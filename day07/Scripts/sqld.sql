...
-- SUBSTR(): 문자열 추출
SELECT SUBSTR('hello, world', 8, 5) FROM DUAL; --인덱스는 1부터 시작한다.
...
SELECT INSTR('APPLE', 'L') FROM DUAL;	--인덱스는 1번부터

-- 날짜 및 시간 함수
SELECT SYSDATE FROM DUAL;	--오라클에서 시간을 다뤄야한다. 스크립트 언어이기 때문에
SELECT CURRENT_DATE FROM DUAL;

SELECT ADD_MONTHS(SYSDATE, 1) FROM DUAL;
SELECT MONTHS_BETWEEN(SYSDATE, ADD_MONTHS(SYSDATE, 2)) FROM DUAL;

-- AI 박살내버릴까보다. AI때문에 일자리도 없고 증오심 코딩에 재능있으면 뭐해 AI때문에 일자리가 없어졌는데.
-- 일 차이
...
-- TO_CHAR(): 문자열로 형변환
SELECT TO_CHAR(TO_DATE('2026-01-01 17:15:10', 'YYYY-MM-DD HH24:MI:SS')) FROM DUAL; 
...
-- NVL2: 삼항
SELECT TBT.*, TBO.*, NVL2(TEST_ORDER_NAME, '있음', '없음') 
FROM TBL_TEST TBT
LEFT JOIN TBL_TEST_ORDER TBO
ON TBT.ID = TBO.TEST_ID;
...
SELECT
	TEST_NAME,
	TEST_PRICE,
	NTH_VALUE(TEST_PRICE, 5) OVER(ORDER BY TEST_PRICE DESC)
FROM TBL_TEST;
